...
public class ChunkOcclusionDataBuilder {
    ...
    private Set<EnumFacing> getOpenFaces(int pos) {
    ...
    private void addEdgeFaces(int pos, Set<EnumFacing> openFaces) {
        final int j = pos >> 0 & 15;
        if (j == 0) {
            openFaces.add(EnumFacing.WEST);
        } else if (j == 15) {
            openFaces.add(EnumFacing.EAST);
        }

        final int k = pos >> 8 & 15;
        if (k == 0) {
            openFaces.add(EnumFacing.DOWN);
        } else if (k == 15) {
            openFaces.add(EnumFacing.UP);
        }

        final int l = pos >> 4 & 15;
        if (l == 0) {
            openFaces.add(EnumFacing.NORTH);
        } else if (l == 15) {
            openFaces.add(EnumFacing.SOUTH);
        }
    }

    private int offset(int pos, EnumFacing arg) {
        return switch (arg) {
            case DOWN -> {
                if ((pos >> 8 & 15) == 0) {
                    yield -1;
