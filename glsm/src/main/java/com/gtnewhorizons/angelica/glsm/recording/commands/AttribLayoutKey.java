...
/**
 * Value key describing a tight interleaved vertex layout (locations + sizes + types +
 * normalized flags). Also computes the aligned per-attrib offsets and stride of the
 * packed layout used for baked vertex data.
 */
public final class AttribLayoutKey {
    ...
    public AttribLayoutKey(int[] locations, int[] sizes, int[] types, boolean[] normalized) {
        ...
        int maxAlign = 1;
        for (int i = 0; i < n; i++) {
            final int typeSize = GLTypes.sizeBytes(types[i]);
