...
/**
 * Compile-time bake of one indexed draw: the bound VAO's attrib data over the tight
 * [minVtx..maxVtx] window plus the (rebased, widened to UINT) index data. GL_QUADS is
 * triangulated at record time. {@link #create} returns null on any recoverable
 * validation failure; callers must not record the placeholder then.
 */
