...
public class AutoShaderUpdater {
    ...
    public void registerShaderReload(ShaderProgram program, ResourceLocation vertexShader,
        ...
        if (!vertexShader.getNamespace().equals(fragmentShader.getNamespace())) {
                    ...
                    for (WatchEvent<?> event : key.pollEvents()) {
                        ...
                        for (ListeningShader shader : listeningShaders) {
                            if (shader.fragmentPath.getFileName().toString().equals(file)
                                ...
                                Minecraft.getMinecraft().addScheduledTask(shader);
                            }
                        }
                    }
                    key.reset();
            ...
            try {
                ...
                if (Minecraft.getMinecraft().player != null) {
                    Minecraft.getMinecraft().player.sendMessage(
                            new TextComponentString("Updated Shaders " + rawVertexPath + " and " + rawFragmentPath));
                }
            } catch (Exception e) {
                e.printStackTrace();
                if (Minecraft.getMinecraft().player != null) {
                    Minecraft.getMinecraft().player.sendMessage(
                            new TextComponentString(
                                    TextFormatting.RED + "Failed to update "
                                            + rawVertexPath
                                            + " or "
                                            + rawFragmentPath
