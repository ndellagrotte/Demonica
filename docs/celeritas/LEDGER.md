...
| `MixinGuiOptions` (opened RSO from the Video Settings button) | not needed; until 0.3.0 `OptionsScreens`, a `GuiOpenEvent` listener | Upstream's own `MixinGuiOptions` already cancels that button to open its screen, which lists Demonica's settings. Until 0.3.0 Demonica swapped that screen for Reese's Sodium Options when it was displayed, since a second cancelling HEAD injection on the same method would race upstream's. RSO is gone since 0.3.0 (`docs/SCOPE_RESEARCH.md`, 4.1 C), and the mod list's Config button (`@Mod(guiFactory)`, `DemonicaGuiFactory`) opens upstream's screen too. |
| The fork's option-API extensions (`Option.getAppliedValue`/`getDefaultValue`/`resetToDefault`/`shouldHideControl`, `OptionImpl.setDefaultValue`, `OptionGroup` names, slider getters, `CyclingControl.getAllowedValues`, `ExternalPage`, `ExternalButtonControl`, `ControlValueFormatter.translateDisabledOrVariable`) | dropped with RSO in 0.3.0; until then O1 for the slider and cycling getters, the rest in RSO's package | Only RSO read them. Upstream's screen draws its own controls, undoes with `Option.reset()` and skips pages without groups. Its own "Shader Packs" tab (`ShaderModBridge`, which finds Demonica's `IrisApi`) replaces Iris's `ExternalPage`. |
| `ActiniumGameOptionPages` (the fork's copies of the General, Quality and Advanced pages) | `DemonicaOptionPages`, listeners on Celeritas's construction events | Upstream's pages are shown as upstream builds them; Demonica's settings are added into their groups (`OptionGroupConstructionEvent`), at the end of Advanced (`OptionPageConstructionEvent`), and as pages (`OptionGUIConstructionEvent`: Iris's and the Debug page). Upstream's fast-renderer toggle, which only sets the static flag S13 overrides, is replaced by Demonica's. The fork-only multidraw mode is gone, and since 0.3.0 so are Fullscreen Mode, the loading-screen frame limit, dynamic FOV and the biome colour noise settings, which nothing applied on upstream (Actinium did in its forked biome colour cache). |
| `ActiniumOptionHost` (pages collected once per game) | dropped with RSO in 0.3.0; until then `DemonicaOptionHost`, collected for each screen | It collected the pages for RSO's screen. Upstream's screen builds its pages for every screen, so slider ranges (GUI scale, chunk threads) and tooltips that depend on the window or the shader pack are current. |
| `OptionGUIConstructionBridge` (each listener isolated and rolled back) | dropped with RSO in 0.3.0 | Upstream's `EventHandlerRegistrar` has no per-listener dispatch: a throwing listener ends the loop, on upstream's screen as it did on RSO's. |
| RSO's text conversion (the first translation key) | dropped with RSO in 0.3.0 | Upstream's screen resolves its own fallback keys for keys 1.12.2 lacks (the default graphics quality's `options.gamma.default`). |
...
| The vanilla draw paths (`MixinTessellator`, the two uploaders, `MixinVertexBuffer`, `BufferBuilderMixin`'s overwrites) | ported in Phases 3 and 4 | Upstream patches none of `Tessellator`, `BufferBuilder`, `VertexBuffer` or the uploaders, so none of the conflict rules applies. They are how vanilla's draws reach the core profile (`VanillaBufferBuilderRenderer`, `VanillaVertexBufferRenderer`). |
| Actinium's fast paths on top of them: deferred particle batching (`DeferredDrawBatcher`), `BufferBuilderStreamingDrawer`, `ModelRendererIrisMixin`'s batching and display lists, the fast lit item path (`ForgeHooksClientIrisMixin`, `FastLitItemDisplayListCache`) | ported in Phases 3 and 4; dropped in 0.3.0 | A shader pack needs none of them, and Celeritas is the performance mod on this stack. BufferBuilder draws take `VanillaBufferBuilderRenderer`'s per-format VAO and VBO, entity models vanilla's `ModelRenderer`, items Forge's `renderLitItem`: the paths they took under a pack. |
| `MixinFontRenderer` and Angelica's batching font renderer (`com.gtnewhorizons.angelica.client.font`, `FontConfig`, `FontRendererAccessor`) | ported (core config); dropped in 0.3.0 | Vanilla's `FontRenderer` draws through GLSM's immediate-mode path, as it did before whenever NeoFontRender was installed. Upstream does not patch `FontRenderer`. The debug switches are `-Ddemonica.fontDebug` and `-Ddemonica.disableFontBatcher` (Actinium's `actinium.*`). `NeoFontRenderCompat` (NeoFontRender's colour palette) is mod compat, Phase 9. |
| `MixinGuiMainMenu` and `PanoramaRenderer` | ported (core config); dropped in 0.3.0 | Vanilla's panorama draws through the redirector. Upstream does not patch `GuiMainMenu`. The panorama draws through Celeritas's `GlProgram`, but its matrix uniform is Demonica's `GlUniformJomlMatrix4f`: Celeritas's `GlUniformMatrix4f` takes the relocated JOML (see `CeleritasJoml`). `JomlBoundaryTest` now scans the mod's own classes as well as the shader tree. |
| `MixinRenderGlobalDestroyBlock`, `MixinTextureManagerDebugLabels`, `MixinEntityRendererDynamicFov` | ported (core config); `MixinEntityRendererDynamicFov` dropped in 0.3.0 | Their methods (`drawBlockDamageTexture`, `TextureManager.loadTexture`, `updateFovModifierHand`) are not touched by upstream's `RenderGlobalMixin` or options mixins. |
...
| `NeverEnoughAnimationsAlphaOverride` | ported; dropped in 0.3.0 | It kept NeverEnoughAnimation's GUI fade visible on the fast lit item path, which is gone. |
...
| `NeoFontRenderCompat` | ported; dropped in 0.3.0 with the batching font renderer | It registered a palette provider that passed the vanilla font renderer's colour codes through. |
...
- **Upstream:** `BiomeColorHelperMixin` is an `@Overwrite` at priority 1200
  ...
  private). Demonica dropped its unused settings for it in 0.3.0 ([Port decisions](#port-decisions)).
