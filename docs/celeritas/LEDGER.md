...
- No `@Redirect` and no `@Overwrite`: HEAD/RETURN injections, `@ModifyArg`,
  `@ModifyVariable`, `@WrapOperation`, `@Accessor`/`@Shadow`.
- The config is non-fatal (`"required": false`, `"defaultRequire": 0`), so a
  ...
  `QuarantinePlugin` logs one line per applied patch and names every injector
  that found no target: `Applied <mixin> to <class>: n of n injectors found
  their targets`. A dev run lists all nine lines; anything short of `n of n` is
  a moved anchor.
...
| SHADOW | I2; from Phase 5: S1, S3, S6s, S7, S16, I1 | Shaders without terrain shadows (L1). |
...
Verified in the dev client on 2026-09-23: all nine mixins apply, and every one of
their 22 injectors finds its target (the `QuarantinePlugin` lines above; the
bytecode exported with `-PmixinExport` shows S8's `@ModifyArg` before
`drawChunkLayer` and I2's `@ModifyVariable` at the head of both searches).
