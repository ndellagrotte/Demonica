...
| COMPAT | C1 | The mods it serves lose that part of their rendering; see the row. Nothing else changes. |
...
| The fork fixes inside those duplicates: biome colour noise in `MixinBiomeColorHelper` (and the fork's biome colour cache), the chunk tracker's per-tick reconcile in `MixinClientChunkManager`, the sprite transparency loop kept to int arithmetic in `features.mipmaps.MixinTextureAtlasSprite` (C2 compiled the polymorphic loop without bound under ZGC; Actinium #171) | lost; upstream candidates | Each sits in a method that an upstream mixin owns (an `@Overwrite` at priority 1200, a RETURN injection, a HEAD injection whose loop is a private method of the mixin), so Demonica cannot take them over without patching upstream's mixin classes. Phase 10 lists them in this ledger as fixes to propose. |
